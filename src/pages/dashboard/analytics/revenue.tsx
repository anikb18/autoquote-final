  ...
  const { data: performanceData } = useQuery<PerformanceData[]>({
    ...
    queryFn: async (): Promise<PerformanceData[]> => {
      ...
      const monthlyData = salesData.reduce((acc: Record<string, any>, curr) => {
        const month = new Date(curr.transaction_date).toLocaleString(
          "default",
          { month: "short" },
        );
      ...
      quotesData.forEach((quote) => {
        const month = new Date(quote.created_at).toLocaleString("default", {
          month: "short",
        });
      ...
      return Object.entries(monthlyData).map(
        ([month, data]: [string, any]): PerformanceData => ({
          period: month,
